...
def create_parser():
    ...
    parser.add_argument('--config_file', '-c', type=str,
                        default='./configs/sc_depth/ddad.py',
                        help='额外的配置文件所在路径')
    ...
    # 数据集设置
    parser.add_argument('--folder_type', type=str,
                        default='sequence', choices=['sequence', 'pair'],
                        help='数据集类型(序列、图像对)')
    ...
    parser.add_argument('--log_step', type=int,
                        default=1,
                        help='设置每隔多久显示一次日志')
    parser.add_argument('--log_summary', type=str,
                        default='summary_log.csv',
                        help='训练过程中的统计信息存储路径,具体为{work_dir}/{exp_name}/{log_summary}')
    parser.add_argument('--log_full', type=str,
                        default='full_log.csv',
                        help='每次梯度下降时的训练统计信息存储路径,具体为{work_dir}/{exp_name}/{log_full}')
