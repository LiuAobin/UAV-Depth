...
def crawl_folder(folder, dataset='ddad'):
    """
    ...
    """
    # 加载图像文件
    imgs = sorted(folder.joinpath('color').files('*.png') + folder.joinpath('color').files('*.jpg'))
    # 根据不同数据集加载不同深度图文件
    if dataset in ['nyu']:  # NYU数据集使用PNG格式的深度图
        depths = sorted(folder.joinpath('depth').files('*.png'))
    elif dataset in ['kitti']:  # KITTI使用NPY格式的深度图
        depths = sorted(folder.joinpath('depth').files('*.npy'))
    elif dataset in ['ddad']:  # DDAD使用NPZ格式数据集
        depths = sorted(folder.joinpath('depth').files('*.npz'))
    else:
...
def crawl_folders(folders_lost, dataset='nyu'):
    """
    遍历文件夹列表，加载图像和对应的深度图
    ...
    for folder in folders_lost:
        ...
        # 根据数据集类型选择不同的深度图文件格式
        if dataset in ['nyu', 'bonn', 'tum']:  # PNG格式深度图
            current_depth = sorted(folder.joinpath('depth').files('*.png'))
        elif dataset in ['ddad','kitti']:  # NPZ格式深度图
            current_depth = sorted(folder.joinpath('depth').files('*.npz'))
